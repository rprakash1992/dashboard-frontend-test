...
const initiateFileUpload = async (file: File, uniqueId: string) => {
  ...
  const resp = await fetch(`${FILE_UPLOAD_ENDPOINT_URL}/initiate-file-upload`, {
    method: "POST",
...
export const multipartUpload = async (
  ...
  try {
    const id = await initiateFileUpload(file, uniqueId);
    const { error } = await setUploadStatusInFileSource(
      metadataId,
      // id,
      false
    );
    ...
    if (id && !error) {
      ...
      if (parts) {
        ...
        while (
          myParts?.filter((part: PartType) => part?.status === "upload_pending")
            ?.length > 0
        ) {
